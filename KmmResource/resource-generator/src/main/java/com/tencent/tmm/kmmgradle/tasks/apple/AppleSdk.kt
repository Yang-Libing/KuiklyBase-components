...
internal object AppleSdk {
    ...
    fun defineNativeTargets(platform: String, archs: List<String>): List<KonanTarget> {
        ...
        when {
            platform.startsWith("iphoneos") -> targets.addAll(archs.map { arch ->
                when (arch) {
            ...
            platform.startsWith("iphonesimulator") -> targets.addAll(archs.map { arch ->
                when (arch) {
            ...
            platform.startsWith("watchos") -> targets.addAll(archs.map { arch ->
                when (arch) {
            ...
            platform.startsWith("watchsimulator") -> targets.addAll(archs.map { arch ->
                when (arch) {
            ...
            platform.startsWith("appletvos") -> targets.addAll(archs.map { arch ->
                when (arch) {
            ...
            platform.startsWith("appletvsimulator") -> targets.addAll(archs.map { arch ->
                when (arch) {
            ...
            platform.startsWith("macosx") -> targets.addAll(archs.map { arch ->
                when (arch) {
